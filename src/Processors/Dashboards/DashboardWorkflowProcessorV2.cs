...
internal class DashboardWorkflowProcessorV2 : IProcessor<CreateDashboardWorkflowCmd, DashboardWorkflowRes>
{
	...
	)
	{
		...
		this._fnFactory = _fnFactory ?? throw new ArgumentNullException(nameof(fnFactory));
	...
	private void BuildDimensions(CreateDashboardWorkflowCmd cmd)
	{
		...
		this._childHash = rootGrid
			...
			.SelectMany((r, i) =>
			{
				List<IRectangleBlueprint> blueprints = new List<IRectangleBlueprint>
				{
	...
	)
	{
		...
		var dashboard =
			this._nodeBuilderFactory.Create()
				.Configure(c => c.AddContext(context).MatchArg(dashboardCmd))
	...
	)
	{
		var qry = 
			this._nodeBuilderFactory.Create()
				.Configure(c => c.MatchArg(cmd).AddContext(context))
				.AddFn(this._queryFn)
				.Build();

		var widget = 
			this._nodeBuilderFactory.Create()
				.Configure(c => c.MatchArg(qry).AddContext(context))
